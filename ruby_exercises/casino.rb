...
def leave
  ...
  if (@wallet - @initial_amount - @total_loses) >= 0
    ...
    p "Your total loses are #{(@wallet - @initial_amount - @total_loses).abs}"
  end
end
...
def accio_money
  p "you are pelted by money from all directions"
  @wallet += 1000000
  next_move
end

def secretly_harry
  p "you have chosen to inherit your parent's money"
  @wallet += 50000
  next_move
end
