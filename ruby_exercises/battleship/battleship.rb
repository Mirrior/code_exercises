...
@shot_board = @computer.clone
@player = @computer.clone
@com_shot_board = @computer.clone
...
def input
  ...
  if coordinates =~ /\A[A-J][0-9]\z|\A[A-J]10\z/
    ...
    puts "Please write input in the form of LetterNumber, ex: A7"
...
def comp_input 
  shot = @com_shot_board.keys.sample
  p "Computer has shot #{shot}"
  @com_shot_board.delete(shot)
  shot
end
...
def cpu_turn
  narrator("17")
  which_sunk(hit?(@player, comp_input))
...
def declare_winner
  if @computer_sunk_ships.count == 5
    puts "#{@user_country} wins! Player wins! "
    @game_end = true
  elsif @player_sunk_ships.count == 5
